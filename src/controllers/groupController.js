...
export class GroupController {
  ...
  async renderGroupsAndProjectsPage (req, res, next) {
    try {
      // Pass the request object to service methods to access the session token
      const dataPromise = this.groupService.getTopGroupsAndSubgroups(req)
      const totalGroupsPromise = this.groupService.getTotalGroups(req)
      ...
      const groups = await Promise.all(data.data.currentUser.groups.edges.map(async (edge) => {
        try {
          ...
          const projectCount = await this.groupService.getProjectCount(edge.node.id, req)
